...
function [upper_bounds,time,Errors]=dual_sddp_control_rhs(T,size_x,size_u,size_b,size_f,upper_x,lower_x,upper_u,lower_u,costc,costd,bs,fs,probabilities,nb_scenarios_rhs,nb_iter_max,lower_pi,upper_pi,a_subi,a_subj,a_valij,b_subi,b_subj,b_valij,c_subi,c_subj,c_valij,d_subi,d_subj,d_valij,e_subi,e_subj,e_valij,f_subi,f_subj,f_valij,big_m,penaltiesQ,penaltiesR,large_bound)
...
dynamic_subi_af{1,1}=[c_subj{1,1},f_subj{1,1}];
dynamic_subj_af{1,1}=[c_subi{1,1},f_subi{1,1}+size_b(1)];
dynamic_valij_af{1,1}=[c_valij{1,1},f_valij{1,1}];

dynamic_subi_af{1,1}=[dynamic_subi_af{1,1},[1:size_u(1)],[1:size_u(1)]];
dynamic_subj_af{1,1}=[dynamic_subj_af{1,1},[size_b(1)+size_f(1)+2*size_x(1)+1:size_b(1)+size_f(1)+2*size_x(1)+2*size_u(1)]];
dynamic_valij_af{1,1}=[dynamic_valij_af{1,1},ones(1,2*size_u(1))];

dynamic_subi_af{1,1}=[dynamic_subi_af{1,1},size_u(1)+1];
dynamic_subj_af{1,1}=[dynamic_subj_af{1,1},size_b(1)+size_f(1)+2*(size_x(1)+size_u(1))+1];
dynamic_valij_af{1,1}=[dynamic_valij_af{1,1},1];

dynamic_buc{1,1}=[costd{1,1};big_m];
dynamic_blc{1,1}=[costd{1,1};-inf];

dynamic_blx{1,1}=[lower_pi{1,1};-large_bound*ones(size_f(1),1)];
dynamic_bux{1,1}=[upper_pi{1,1};zeros(size_f(1),1)];

dynamic_c{1,1}=[bs{1,1}{1,1};fs{1,1}{1,1}];
for i=1:size_x(1)
...
for i=1:nb_scenarios_rhs(T)
    ...
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},a_subi{1,T}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T))),c_subi{1,T}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},a_valij{1,T},c_valij{1,T}];
end

for i=1:nb_scenarios_rhs(T)
    dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},d_subj{1,T}+(i-1)*size_x(T),f_subj{1,T}+(i-1)*size_u(T)+nb_scenarios_rhs(T)*size_x(T)];
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},d_subi{1,T}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T),f_subi{1,T}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},d_valij{1,T},f_valij{1,T}];
end

for i=1:nb_scenarios_rhs(T)
    dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},b_subj{1,T-1}+nb_scenarios_rhs(T)*(size_x(T)+size_u(T)),e_subj{1,T-1}+nb_scenarios_rhs(T)*(size_x(T)+size_u(T))];
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},b_subi{1,T-1}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T))),e_subi{1,T-1}+(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},probabilities{1,T-1}(i)*b_valij{1,T-1},probabilities{1,T-1}(i)*e_valij{1,T-1}];
end

for i=1:nb_scenarios_rhs(T)
    dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},[(i-1)*size_x(T)+1:i*size_x(T)]];
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+size_f(T)+1:(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+size_f(T)+size_x(T)]];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},ones(1,size_x(T))];
    
    dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},[(i-1)*size_x(T)+1:i*size_x(T)]];
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+size_x(T)+size_f(T)+1:(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+size_f(T)+2*size_x(T)]];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},ones(1,size_x(T))];
    ...
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+2*size_x(T)+size_f(T)+1:(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+2*size_x(T)+size_f(T)+size_u(T)]];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},ones(1,size_u(T))];
    
    dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},[nb_scenarios_rhs(T)*size_x(T)+(i-1)*size_u(T)+1:nb_scenarios_rhs(T)*size_x(T)+i*size_u(T)]];
    dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[(i-1)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_b(T)+2*size_x(T)+size_f(T)+1+size_u(T):i*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))]];
    dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},ones(1,size_u(T))];    
end
...
dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[nb_scenarios_rhs(T)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+1:nb_scenarios_rhs(T)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_x(T-1)]];
dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},ones(1,size_x(T-1))];

dynamic_subi_af{1,T}=[dynamic_subi_af{1,T},[nb_scenarios_rhs(T)*(size_x(T)+size_u(T))+1:nb_scenarios_rhs(T)*(size_x(T)+size_u(T))+size_x(T-1)]];
dynamic_subj_af{1,T}=[dynamic_subj_af{1,T},[nb_scenarios_rhs(T)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+size_x(T-1)+1:nb_scenarios_rhs(T)*(size_b(T)+size_f(T)+2*(size_x(T)+size_u(T)))+2*size_x(T-1)]];
dynamic_valij_af{1,T}=[dynamic_valij_af{1,T},-ones(1,size_x(T-1))];
...
for i=1:nb_scenarios_rhs(T)
    dynamic_blx{1,T}=[dynamic_blx{1,T};lower_pi{1,T};-large_bound*ones(size_f(T),1)];
    dynamic_bux{1,T}=[dynamic_bux{1,T};upper_pi{1,T};zeros(size_f(T),1)];
    dynamic_c{1,T}=[dynamic_c{1,T};probabilities{1,T-1}(i)*bs{1,T}{1,i};probabilities{1,T-1}(i)*fs{1,T}{1,i}];    
    for j=1:size_x(T)
...
for t=2:T-1
    ...
    dynamic_subj_af{1,t}=[];
    dynamic_valij_af{1,t}=[];
    dimC=size_b(t)+size_f(t)+2*(size_x(t)+size_u(t));
    for i=1:nb_scenarios_rhs(t)
        ...
        dynamic_subi_af{1,t}=[dynamic_subi_af{1,t},(i-1)*size_u(t)+f_subj{1,t}];
        dynamic_subj_af{1,t}=[dynamic_subj_af{1,t},(i-1)*dimC+size_b(t)+f_subi{1,t}];
        dynamic_valij_af{1,t}=[dynamic_valij_af{1,t},f_valij{1,t}];
        
        dynamic_subi_af{1,t}=[dynamic_subi_af{1,t},[(i-1)*size_u(t)+1:i*size_u(t)]];
        dynamic_subj_af{1,t}=[dynamic_subj_af{1,t},[(i-1)*dimC+size_b(t)+size_f(t)+2*size_x(t)+1:(i-1)*dimC+size_b(t)+size_f(t)+2*size_x(t)+size_u(t)]];
        dynamic_valij_af{1,t}=[dynamic_valij_af{1,t},ones(1,size_u(t))];
        
        dynamic_subi_af{1,t}=[dynamic_subi_af{1,t},[(i-1)*size_u(t)+1:i*size_u(t)]];
        dynamic_subj_af{1,t}=[dynamic_subj_af{1,t},[(i-1)*dimC+size_b(t)+size_f(t)+2*size_x(t)+size_u(t)+1:(i-1)*dimC+size_b(t)+size_f(t)+2*(size_x(t)+size_u(t))]];
        dynamic_valij_af{1,t}=[dynamic_valij_af{1,t},ones(1,size_u(t))];
        ...
        dynamic_subi_af{1,t}=[dynamic_subi_af{1,t},nb_scenarios_rhs(t)*size_u(t)+e_subj{1,t}];
        dynamic_subj_af{1,t}=[dynamic_subj_af{1,t},(i-1)*dimC+size_b(t)+e_subi{1,t}];
        dynamic_valij_af{1,t}=[dynamic_valij_af{1,t},probabilities{1,t-1}(i)*e_valij{1,t}];
...
while (iter<=nb_iter_max)
    ...
    prob.a = sparse(dynamic_subi_af{1,1},dynamic_subj_af{1,1},dynamic_valij_af{1,1},size_u(1)+iter,size_b(1)+size_f(1)+1+2*(size_x(1)+size_u(1)));
    ...
    elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
        ...
        mus{1,1}=sol(size_b(1)+1:size_b(1)+size_f(1));
        nus1{1,1}=sol(size_b(1)+size_f(1)+1:size_b(1)+size_f(1)+size_x(1));
        nus2{1,1}=sol(size_b(1)+size_f(1)+size_x(1)+1:size_b(1)+size_f(1)+2*size_x(1));
    end
    
    for t=2:T-1
         clear prob;
         dimCs=nb_scenarios_rhs(t)*(size_b(t)+size_f(t)+2*(size_x(t)+size_u(t))+1);
         ...
         aux=costc{1,t-1}-sparse(a_subj{1,t-1},a_subi{1,t-1},a_valij{1,t-1},size_x(t-1),size_b(t-1))*pis{1,t-1}-sparse(d_subj{1,t-1},d_subi{1,t-1},d_valij{1,t-1},size_x(t-1),size_f(t-1))*mus{1,t-1}-nus1{1,t-1}-nus2{1,t-1};
         ...
         prob.a =sparse(dynamic_subi_af{1,t},dynamic_subj_af{1,t},dynamic_valij_af{1,t},nb_scenarios_rhs(t)*(iter+size_u(t))+size_x(t-1),nb_scenarios_rhs(t)*(1+size_b(t)+size_f(t)+2*(size_x(t)+size_u(t)))+2*(size_x(t-1)));
         [~,res]=mosekopt('maximize echo(0)',prob);
        sol=res.sol.bas.xx;
        solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
        if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
        ...
        elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
            ...
            mus{1,t}=sol((Index-1)*dimC+size_b(t)+1:(Index-1)*dimC+size_b(t)+size_f(t));
            nus1{1,t}=sol((Index-1)*dimC+size_b(t)+size_f(t)+1:(Index-1)*dimC+size_b(t)+size_f(t)+size_x(t));
            nus2{1,t}=sol((Index-1)*dimC+size_b(t)+size_f(t)+size_x(t)+1:(Index-1)*dimC+size_b(t)+size_f(t)+2*size_x(t));
        end
    end
    ...
    for t=T:-1:2
        if (t==T)
            ...
            elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                ...
                slopemu=-sparse(d_subi{1,T-1},d_subj{1,T-1},d_valij{1,T-1},size_f(T-1),size_x(T-1))*daux;
                slopenu1=-daux;
                slopenu2=-daux;
                intercept=prob.buc'*(dual1-dual2)-slopepi'*pis{1,T-1}-slopemu'*mus{1,T-1}-slopenu1'*nus1{1,T-1}-slopenu2'*nus2{1,T-1};
                for i=1:nb_scenarios_rhs(T)
            ...
            prob.blc=dynamic_blc{1,t};
            prob.buc=dynamic_buc{1,t};
            prob.a = sparse(dynamic_subi_af{1,t},dynamic_subj_af{1,t},dynamic_valij_af{1,t},nb_scenarios_rhs(t)*(iter+size_u(t)+1)+size_x(t-1),nb_scenarios_rhs(t)*(1+size_b(t)+size_f(t)+2*(size_x(t)+size_u(t)))+2*size_x(t-1));            
            [~,res]=mosekopt('maximize echo(0)',prob);
            sol=res.sol.bas.xx;
            solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
            if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
            ...
            elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                ...
                slopenu1=-daux;
                slopenu2=-daux;
                intercept=prob.buc(1:nb_scenarios_rhs(t)*size_u(t)+size_x(t-1))'*(dual1(1:nb_scenarios_rhs(t)*size_u(t)+size_x(t-1))-dual2(1:nb_scenarios_rhs(t)*size_u(t)+size_x(t-1)))-slopepi'*pis{1,t-1}-slopemu'*mus{1,t-1}-slopenu1'*nus1{1,t-1}-slopenu2'*nus2{1,t-1};
        ...
        if (t==2)
            row=size_u(1)+iter+1;
            dynamic_subi_af{1,1}=[dynamic_subi_af{1,1},row*ones(1,size_b(1)),row*ones(1,size_f(1)),row*ones(1,size_x(1)),row*ones(1,size_x(1)),row];
            dynamic_subj_af{1,1}=[dynamic_subj_af{1,1},[1:size_b(1)],[size_b(1)+1:size_b(1)+size_f(1)],[size_b(1)+size_f(1)+1:size_b(1)+size_f(1)+size_x(1)],[size_b(1)+size_f(1)+size_x(1)+1:size_b(1)+size_f(1)+2*size_x(1)],size_b(1)+size_f(1)+2*(size_x(1)+size_u(1))+1];
            dynamic_valij_af{1,1}=[dynamic_valij_af{1,1},-slopepi',-slopemu',-slopenu1,-slopenu2,1];
            dynamic_buc{1,1}=[dynamic_buc{1,1};intercept];
            dynamic_blc{1,1}=[dynamic_blc{1,1};-inf];
        else
            dimC=size_b(t-1)+size_f(t-1)+2*(size_x(t-1)+size_u(t-1));
            ...
            for i=1:nb_scenarios_rhs(t-1)
                ...
                dynamic_subi_af{1,t-1}=[dynamic_subi_af{1,t-1},row*ones(1,size_b(t-1)),row*ones(1,size_f(t-1)),row*ones(1,size_x(t-1)),row*ones(1,size_x(t-1)),row];
                dynamic_subj_af{1,t-1}=[dynamic_subj_af{1,t-1},[(i-1)*dimC+1:(i-1)*dimC+size_b(t-1)],[(i-1)*dimC+size_b(t-1)+1:(i-1)*dimC+size_b(t-1)+size_f(t-1)],[(i-1)*dimC+size_b(t-1)+size_f(t-1)+1:(i-1)*dimC+size_b(t-1)+size_f(t-1)+size_x(t-1)],[(i-1)*dimC+size_b(t-1)+size_f(t-1)+size_x(t-1)+1:(i-1)*dimC+size_b(t-1)+size_f(t-1)+2*size_x(t-1)],nb_scenarios_rhs(t-1)*dimC+i];
                dynamic_valij_af{1,t-1}=[dynamic_valij_af{1,t-1},-slopepi',-slopemu',-slopenu1',-slopenu2',1];
            end
        end
    end
    ...
    prob.blc=dynamic_blc{1,1};
    prob.buc=dynamic_buc{1,1};
    prob.a = sparse(dynamic_subi_af{1,1},dynamic_subj_af{1,1},dynamic_valij_af{1,1},size_u(1)+iter+1,size_b(1)+size_f(1)+1+2*(size_x(1)+size_u(1)));
    [~,res]=mosekopt('maximize echo(0)',prob);
    sol=res.sol.bas.xx;
    solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
    if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
